...
void Send_Byte(
    ...
    int nbit1,
    bool
        msb);  // 定义发送原始数据的函数，data1(数据），nbit1(数据二进制位数），msb(数据发送顺序，1为低位先发，0为高位先发）

void Send_Code_S(uint8_t A,
                 uint8_t B,
                 uint8_t C,
                 uint8_t D_1,
                 uint8_t D_2,
...
void IRsendMeidi::begin_2()
{  // 初始化IRsend.begin的函数，需写入到主程序的void setup（）中。
...
IRsendMeidi::IRsendMeidi(uint8_t ir_led)
{  // 返回发射信号的引脚

    kIrLed = ir_led;
}

void IRsendMeidi::setZBPL(int khz)
{  // 定义红外发射的载波频率

    ZBPL = khz;
}

void IRsendMeidi::setTemps(float Temps1)
{  // 设置空调温度
...
void IRsendMeidi::setModes(int Modes1)
{  // 设置空调模式。
...
void IRsendMeidi::setFanSpeeds(int FanSpeeds1)
{  // 设置空调风速。
...
void IRsendMeidi::setEco(bool Eco)
{  // 开关ECO模式
...
void IRsendMeidi::setPowers(bool Powers)
{  // 开关空调
...
void IRsendMeidi::setSwingUD(bool SwingUD)
{  // 开关上下扫风
...
void IRsendMeidi::setSwingLR(bool SwingLR)
{  // 开关左右扫风
...
void IRsendMeidi::setFZC(bool FZC)
{  // 开关防直吹
...
void IRsendMeidi::setTimers(float Timers)
{  // 设置定时
...
void IRsendMeidi::Send_Code(uint8_t AC, uint8_t BC, uint8_t CC)
{  // 发送ABC码的函数
...
void Send_Meidi(bool Code_State)
{  // 发送长码数据的函数，1为正常发送，0为C反码固定为0xFF的发送。
...
void Send_Byte(uint8_t data1, int nbit1, bool msb)
{  // 数据发送函数定义

    irsend.enableIROut(ZBPL);

    irsend.sendData(Marks, One_Space, Marks, Zero_Space, data1, nbit1,
